...
# "extension" will usually be c
def process_folder(folders, sought_exceptions=[], includes_per_scu=0, extension="c"):
    if len(folders) == 0:
        return
...
def generate_scu_files(max_includes_per_scu):
    ...
    if folder_not_found("core") or folder_not_found("platform") or folder_not_found("platform"):
        raise RuntimeError("scu_builders.py must be run from the lunar_sprites folder.")
        return
