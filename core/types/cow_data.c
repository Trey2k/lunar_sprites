...
static size_t *get_ref_count(const COWData *cow_data) {
	return (size_t *)(cow_data->data + COW_DATA_REF_COUNT_OFFSET);
}

static uint64 *get_data_size(const COWData *cow_data) {
	return (uint64 *)(cow_data->data + COW_DATA_SIZE_OFFSET);
}
...
static void copy_on_write(COWData *cow_data) {
	...
	uint8 *new_data = ls_malloc(data_size + COW_DATA_DATA_OFFSET);
	ls_memcpy(new_data, cow_data->data, data_size + COW_DATA_DATA_OFFSET);
	(*ref_count)--;
...
void *cowdata_get_data_ptrw(COWData *cow_data) {
	...
	return (void *)(cow_data->data + COW_DATA_DATA_OFFSET);
}
...
const void *cowdata_get_data_ptr(const COWData *cow_data) {
	...
	return (const void *)(cow_data->data + COW_DATA_DATA_OFFSET);
}
...
void cowdata_resize(COWData *cow_data, size_t size, size_t count) {
	if (!cow_data->data) {
		cow_data->data = ls_malloc(size * count + COW_DATA_DATA_OFFSET);
		*get_ref_count(cow_data) = 1;
	...
	*data_size = size * count;
	...
	cow_data->data = ls_realloc(cow_data->data, size * count + COW_DATA_DATA_OFFSET);
}

void cowdata_set(COWData *cow_data, const void *data, size_t size, size_t count) {
	cowdata_resize(cow_data, size, count);

	ls_memcpy(cow_data->data + COW_DATA_DATA_OFFSET, data, size * count);
}
...
uint8 cowdata_get_byte(const COWData *cow_data, size_t index) {
	...
	return cow_data->data[COW_DATA_DATA_OFFSET + index];
}
...
void cowdata_set_byte(COWData *cow_data, size_t index, uint8 byte) {
	...
	copy_on_write(cow_data);
	cow_data->data[COW_DATA_DATA_OFFSET + index] = byte;
}
...
void cowdata_append_byte(COWData *cow_data, uint8 byte) {
	...
	cow_data->data[COW_DATA_DATA_OFFSET + data_size - 1] = byte;
}
